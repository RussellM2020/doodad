...
mode_ec2 = dd.mode.EC2AutoconfigDocker(
        ...
        s3_log_prefix='PointMass',  # Folder to store log files under
        s3_log_name='finnMAML_.5_neg.5',
...
mlr = 0.01 ; seed =1
flr = 1 ; n_parallel = 4
for mbs in [5,10,15, 20]:
    for fbs in [1,5,10,15,20]:

        expName = 'mlr_'+str(mlr)+'_flr_'+str(flr)+'_fbs_'+str(fbs)+'_mbs_'+str(mbs)
        dd.launch_python(
            target=os.path.join(THIS_FILE_DIR, '/home/russellm/maml_rl/maml_examples/maml_pointMass.py'),
