...
mode_ec2 = dd.mode.EC2AutoconfigDocker(
        ...
        s3_log_name='rl2_'+regionSize+'_block_size0.04_mass0.1',
    ...
    for clip_lr in [0.01, 0.05]:
        for numEpisodes in [2,5]:
            for batchSize in [ 50000, 150000]:
                ...
                expName = 'batchSize_'+str(batchSize)+'_numEp_'+str(numEpisodes)+'_clipLR_'+str(clip_lr) +  '_seed'+str(seed)


                dd.launch_python(
                    target=os.path.join(THIS_FILE_DIR, '/home/russellm/rllab-private-RL2unstable/examples/rl2_sawyer_pickPlace_push.py'),
                    #target=os.path.join(THIS_FILE_DIR, str(targetScript) ),  # point to a target script. If running remotely, this will be copied over
                    mode=MY_RUN_MODE,
                    mount_points=mounts,
                    args={
                        'variant': {'saveDir':OUTPUT_DIR+expName+'/', 'seed':seed, 'n_episodes': numEpisodes, 'batch_size': batchSize, 'regionSize': regionSize, 'envType': envType, 'clip_lr': clip_lr},
