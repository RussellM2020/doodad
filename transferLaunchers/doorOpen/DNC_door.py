...
for seed in range(2):
    for numPartitions in [5, 10]:
   
        for step_size in [0.05, 0.01]:
            ...
            for penalty in [0, 0.01, 0.05]:
                ...
                dd.launch_python(
                    target=os.path.join(THIS_FILE_DIR, '/home/russellm/dnc/examples/dnc_door.py'),
                    ...
                    args={
                        'variant': {'numPartitions': numPartitions, 'hidden_sizes': hidden_sizes, 'batch_size': batch_size , 'step_size': step_size, 
                                    'saveDir': OUTPUT_DIR+expName+'/', 'penalty': penalty },
