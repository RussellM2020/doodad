...
mode_ec2 = dd.mode.EC2AutoconfigDocker(
        ...
        s3_log_name='ppoExperts_'+regionSize+ '_block_size0.04_mass0.01_Rew_posPlace',
...
for npSeed in range(2):
    for cliprange in [0.1]:

    #for cliprange in ['custom']:
        for hidden_sizes in [(64, 64)]:
            for batch_size in [2048]:
                for startTask in range(5,6):
                    ...
                    dd.launch_python(
                        ...
                        mount_points=mounts,
                        args={
                            'variant': {'tfSeed': tfSeed, 'npSeed' : npSeed,  'batch_size': batch_size , 'contextual' : contextual , 'enableRotation' : enableRotation, 'rewMode': rewMode,
                                        'hiddenSizes' : hidden_sizes, 'startTask': startTask, 'numTasks':1, 'expName' : expName, 'cliprange': cliprange, 'regionSize': regionSize},
