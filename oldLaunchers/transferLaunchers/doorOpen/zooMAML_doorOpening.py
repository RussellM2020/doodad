...
mode_ec2 = dd.mode.EC2AutoconfigDocker(
        ...
        spot_price=0.5,  # Maximum bid price
        s3_log_prefix='Sawyer_'+prefixEnvName,  # Folder to store log files under
...
for seed in range(2):
    ...
    for rollouts_per_meta_task in [ 20, 50]:
        ...
        for inner_step in [0.01]:
       
            expName = 'fbs_'+str(rollouts_per_meta_task)+'_innerStep_'+str(inner_step)+'_seed_'+str(seed)
            ...
            dd.launch_python(
                ...
                args={
                    'variant': {'rollouts_per_meta_task':rollouts_per_meta_task,  'data_path': OUTPUT_DIR+expName+'/', 'seed': seed, 
                                'init_inner_kl_penalty': inner_step, 'envType' : envType, 'regionSize' : regionSize},
