...
mode_ec2 = dd.mode.EC2AutoconfigDocker(
        ...
        s3_log_name='zooMAML_'+regionSize+'_'+algo,
...
for seed in range(2):
    ...
    #for mlr in 
        for inner_lr in [0.01, 0.05]:
            ...
            #for inner_type in ['log_likelihood']:
            ...
            expName = 'hs_'+h_str+'fbs_'+str(rollouts_per_meta_task)+'_innerLR_'+str(inner_lr)+'_LR_'+str(outer_lr)+'/seed_'+str(seed)

            dd.launch_python(
                target=os.path.join(THIS_FILE_DIR, '/home/russellm/maml_zoo/run_scripts/multiWorld_launcher.py'),
                #target=os.path.join(THIS_FILE_DIR, str(targetScript) ),  # point to a target script. If running remotely, this will be copied over
                mode=MY_RUN_MODE,
                mount_points=mounts,
                args={
                    'variant': {'algo' : algo, 'rollouts_per_meta_task':rollouts_per_meta_task,  'data_path': OUTPUT_DIR+expName+'/', 'seed': seed, 
                                'inner_lr': inner_lr, 'outer_lr': outer_lr, 'envType' : envType, 'regionSize' : regionSize, 'hidden_sizes' : hidden_sizes 
                                },
