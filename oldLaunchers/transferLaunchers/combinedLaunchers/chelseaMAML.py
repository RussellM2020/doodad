...
# envType = 'SawyerMultiDomain-Push-SparseDoor' ; max_path_length = 100 ;
# tasksFile = 'multi_domain/multiDomain_pushDoorDrawer_10each' ; annotation = 'pushDoor_10each' ; total_num_tasks = 20


# envType = 'SawyerMultiDomain-Push-Door-Drawer' ; max_path_length = 100 ;
# tasksFile = 'multi_domain/multiDomain_pushDoorDrawer_10each' ; annotation = '_10each' ; total_num_tasks = 30

#envType = 'humanoid-dir' ; max_path_length = 200 ; annotation = 'restricted_train_set' ; total_num_tasks = 100
envType = 'humanoid-modControl' ; max_path_length = 200 ; annotation = 'dynamics_extrapolation_maml' ; total_num_tasks = 10
tasksFile = 'humanoid/humanoid_dynamics_train'
...
mounts = [
	...
	mount.MountLocal(local_dir="~/rlkit",
							 mount_point="/root/code/rlkit",
							 filter_dir=["__pycache__", ".git"], pythonpath = True),

	mount.MountLocal(local_dir="~/MIER/smrl",
								 mount_point="/root/code/MIER/smrl",
								 filter_dir=["__pycache__", ".git"], pythonpath = True),
			...
			for policyType in ['fullAda_Bias' , 'biasAda_Bias']:
				for ldim in [2,4]:
					...
					dd.launch_python(
						...
						mount_points=mounts,
						args={
							'variant': {'policyType':policyType, 'ldim':ldim, 'init_flr': init_flr, 'seed' : seed , 'tasksFile': tasksFile,
							 'log_dir':OUTPUT_DIR+expName+'/',  'n_parallel' : n_parallel, 'total_num_tasks': total_num_tasks,
							 'envType': envType  , 'fbs' : fbs  , 'mbs' : mbs , 'max_path_length' : max_path_length , 'load_policy': load_policy},
